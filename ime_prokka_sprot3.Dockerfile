...
RUN apt update && \
	...
	rm -rf .git && \
   	git clone https://github.com/karnagowda/prokkadb.git /opt/prokkadb && \
	cd /opt/prokkadb && \
	git checkout && \
	rm -rf .git && \
   	gunzip /opt/prokkadb/sprot/sprot3.gz && \
	rm -rf /opt/prokka/db/kingdom/Bacteria/sprot && \
   	mv /opt/prokkadb/sprot/sprot3 /opt/prokka/db/kingdom/Bacteria/sprot && \
